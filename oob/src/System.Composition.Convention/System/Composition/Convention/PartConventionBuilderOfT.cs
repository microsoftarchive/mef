...
namespace System.Composition.Convention
{
    ...
    public class PartConventionBuilder<T> : PartConventionBuilder
    {
        private class MethodExpressionAdapter
        {
            ...
            {
                this._methodInfo = SelectMethods(methodSelector);
            }

            public bool VerifyMethodInfo(MethodInfo mi)
            {
                return mi == this._methodInfo;
            }
        ...
        private class PropertyExpressionAdapter
        {
            ...
            Action<ImportConventionBuilder> _configureImport;
            Action<ExportConventionBuilder> _configureExport;
            ...
            {
                this._propertyInfo = SelectProperties(propertySelector);
        ...
        private class ConstructorExpressionAdapter
        {
            ...
            Dictionary<ParameterInfo, Action<ImportConventionBuilder>> _importBuilders = null;
                    ...
                    if(argument.NodeType == ExpressionType.Call)
                    {
                        ...
                        if(methodCallExpression.Arguments.Count() == 1)
                        {
                            ...
                            if(parameter.NodeType == ExpressionType.Lambda)
                            {
                                ...
                                if(this._importBuilders == null)
                                {
                                    this._importBuilders = new Dictionary<ParameterInfo, Action<ImportConventionBuilder>>();
                                }
                                this._importBuilders.Add(parameterInfos[index], (Action<ImportConventionBuilder>)importDelegate);
                                ++index;
                            }
                        }
                    }
