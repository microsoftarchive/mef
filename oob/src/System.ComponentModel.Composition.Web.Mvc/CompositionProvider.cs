...
namespace System.ComponentModel.Composition.Web.Mvc
{
    ...
    {
        static ExportLifetimeContext<IExportProvider> _container;
        static RequestScopeFactory _requestScopeFactory;
        ...
        public static void SetConfiguration(ContainerConfiguration configuration)
        {
            ...
            _container = configuration
                .WithPart<RequestScopeFactory>(new RegistrationBuilder())
                .CreateContainer();

            _requestScopeFactory = _container.Value.GetExport<RequestScopeFactory>();

            if (DependencyResolver.Current.GetType().Name != "DefaultDependencyResolver")
                throw new LightweightCompositionException("The Composition Provider for ASP.NET MVC sets the application-wide DependencyResolver. It seems that another dependency resolver has already been set.");
        ...
        internal static IExportProvider Current
        {
            get
            {
                ...
                if (current == null)
                {
                    current = _requestScopeFactory.BeginRequestScope();
        ...
        internal static ExportLifetimeContext<IExportProvider> CurrentInitialisedScope
        {
            get { return (ExportLifetimeContext<IExportProvider>)HttpContext.Current.Items[typeof(CompositionProvider)]; }
