...
namespace System.Reflection.Context
{
    public partial class CustomReflectionContext
    {
        private class ReflectionContextProjector : Projector
        {
            ...
            public Type ProjectTypeIfNeeded(Type value)
            {
                if (NeedsProjection(value))
                {
                    // Map the assembly to the underlying context first
                    if (_context.SourceContext != null)
                        value = _context.SourceContext.MapType(value);
            ...
            public Assembly ProjectAssemblyIfNeeded(Assembly value)
            {
                if (NeedsProjection(value))
                {
                    // Map the assembly to the underlying context first
                    if (_context.SourceContext != null)
                        value = _context.SourceContext.MapAssembly(value);
            ...
            public override Type ProjectType(Type value)
            {
                if (value == null)
                    return null;
            ...
            public override MethodBase ProjectMethodBase(MethodBase value)
            {
                ...
                throw new InvalidOperationException(SR.GetString(SR.InvalidOperation_InvalidMethodType, value.GetType()));
            }
            ...
            public override MemberInfo ProjectMember(MemberInfo value)
            {
                ...
                switch (value.MemberType)
                {
                    ...
                    default:
                        throw new InvalidOperationException(SR.GetString(SR.InvalidOperation_InvalidMemberType, value.Name, value.MemberType));
                }
            ...
            public override InterfaceMapping ProjectInterfaceMapping(InterfaceMapping value)
            {
                ...
                mapping.InterfaceMethods = Project(value.InterfaceMethods, ProjectMethod);
                mapping.InterfaceType = ProjectType(value.InterfaceType);
                mapping.TargetMethods = Project(value.TargetMethods, ProjectMethod);
                mapping.TargetType = ProjectType(value.TargetType);
