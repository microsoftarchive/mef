...
namespace System.Composition.Convention
{
    ...
    public class ConventionBuilder : AttributedModelProvider
    {
        ...
        private List<PartConventionBuilder> _conventions = new List<PartConventionBuilder>();
        ...
        private IEnumerable<Tuple<object, List<Attribute>>> EvaluateThisTypeInfoAgainstTheConvention(TypeInfo typeInfo)
        {
            ...
            foreach (var builder in this._conventions.Where(c => c.SelectType(type)))
            {
        ...
        public override IEnumerable<Attribute> GetCustomAttributes(Type reflectedType, System.Reflection.MemberInfo member)
        {
            ...
            if (typeInfo != null)
            {
                ...
                {
                    this._memberInfos.TryGetValue(memberInfo, out cachedAttributes);
                }
                if (cachedAttributes == null)
                {
                    using (new WriteLock(this._lock))
                    {
                        ...
                        if (!this._memberInfos.TryGetValue(memberInfo, out cachedAttributes))
                        {
                            ...
                            foreach (var element in EvaluateThisTypeInfoAgainstTheConvention(typeInfo))
                            {
                                ...
                                if (attributeList != null)
                                {
                                    ...
                                    if (mi != null)
                                    {
                                        ...
                                        {
                                            if (!this._memberInfos.TryGetValue(mi, out memberAttributes))
                                            {
                                                this._memberInfos.Add(mi, element.Item2);
                                            }
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        if (!this._parameters.TryGetValue(pi, out parameterAttributes))
                                        {
                                            this._parameters.Add(pi, element.Item2);
                                        }
                                    }
                                }
                            }
            ...
            using (new ReadLock(this._lock))
            {
                if (!this._memberInfos.TryGetValue(member, out cachedAttributes))
                {
                    ...
                    if (reflectedType != null
                        && !this._memberInfos.TryGetValue(member.DeclaringType.GetTypeInfo() as MemberInfo, out cachedAttributes))
                    {
        ...
        private List<Attribute> ReadParameterCustomAttributes(Type reflectedType, System.Reflection.ParameterInfo parameter)
        {
            ...
            using (new ReadLock(this._lock))
            {
                if (!this._parameters.TryGetValue(parameter, out cachedAttributes))
                {
                    ...
                    if (reflectedType != null 
                     && !this._memberInfos.TryGetValue(reflectedType.GetTypeInfo() as MemberInfo, out cachedAttributes))
                    {
                        // If there is nothing for this parameter look to see if the declaring Member has been cached yet?
                        // need to do it outside of the lock, so set the flag we'll check it in a bit
                        getMemberAttributes = true;
